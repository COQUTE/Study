...
CREATE TABLE TB_DBA (
	...
	CONSTRAINT DBA_PH_CK CHECK(DBA_PHONE NOT LIKE '%-%')
);
...
CREATE TABLE TB_PROFESSOR (
	...
	CONSTRAINT PROF_PH_CK CHECK(PROF_PHONE NOT LIKE '%-%')
);
...
CREATE TABLE TB_DEPARTMENT (
	...
	CONSTRAINT DEPT_TEL_CK CHECK(DEPT_TEL NOT LIKE '%-%'),
	CONSTRAINT DEPT_FK FOREIGN KEY(HEAD_PROF) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE SET NULL
);
...
CREATE TABLE TB_STUDENT (
	...
	CONSTRAINT STD_MAJOR_FK FOREIGN KEY(MAJOR) REFERENCES TB_DEPARTMENT(DEPT_ID) ON DELETE SET NULL,
	CONSTRAINT STD_MINOR_FK FOREIGN KEY(MINOR) REFERENCES TB_DEPARTMENT(DEPT_ID) ON DELETE SET NULL
);
...
CREATE TABLE TB_DEPT_PROF (
	...
	CONSTRAINT DP_DEPT_FK FOREIGN KEY(DEPT_ID) REFERENCES TB_DEPARTMENT(DEPT_ID) ON DELETE CASCADE,
	CONSTRAINT DP_PROF_FK FOREIGN KEY(PROF_ID) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE CASCADE
);
...
CREATE TABLE TB_MANAGEMENT (
	...
	CONSTRAINT MAN_STD_FK FOREIGN KEY(STD_ID) REFERENCES TB_STUDENT(STD_ID) ON DELETE CASCADE,
	CONSTRAINT MAN_PROF_FK FOREIGN KEY(PROF_ID) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE CASCADE,
	CONSTRAINT MAN_SEM_CK CHECK(SEMESTER IN('1', '2'))
);
...
CREATE TABLE TB_LECTURE (
	...
	LCT_YEAR VARCHAR2(4 BYTE) NOT NULL, -- 강좌 년도
	...
	CONSTRAINT LCT_PK PRIMARY KEY(LCT_ID, CLASS_ID),
	CONSTRAINT LCT_NAME_UQ UNIQUE(LCT_YEAR, LCT_SEMESTER, LCT_NAME),
	CONSTRAINT LCT_CLASS_UQ UNIQUE(DAY, PERIOD, CLASSROOM),
	CONSTRAINT LCT_CREDIT_CK CHECK(CREDIT BETWEEN 1 AND 4),
	CONSTRAINT LCT_HOUR_CK CHECK(HOUR BETWEEN 1 AND 6),
	CONSTRAINT LCT_SEM_CK CHECK(LCT_SEMESTER IN('1', '2')),
	CONSTRAINT LCT_PROF_FK FOREIGN KEY(PROF_ID) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE CASCADE,	
	CONSTRAINT LCT_DEPT_FK FOREIGN KEY(DEPT_ID) REFERENCES TB_DEPARTMENT(DEPT_ID) ON DELETE CASCADE
);
...
CREATE TABLE TB_COURSE_HISTORY (
	...
	CONSTRAINT COURSE_PK PRIMARY KEY(STD_ID, LCT_ID, CLASS_ID, PROF_ID),
	CONSTRAINT COURSE_TOTAL_CK CHECK(TOTAL_SCR BETWEEN 0 AND 100),
	CONSTRAINT COURSE_GRADE_CK CHECK(GRADE BETWEEN 'A' AND 'F'),
	CONSTRAINT COURSE_STD_FK FOREIGN KEY(STD_ID) REFERENCES TB_STUDENT(STD_ID) ON DELETE CASCADE,
	CONSTRAINT COURSE_LCT_FK FOREIGN KEY(LCT_ID, CLASS_ID) REFERENCES TB_LECTURE(LCT_ID, CLASS_ID) ON DELETE CASCADE,
	CONSTRAINT COURSE_PROF_FK FOREIGN KEY(PROF_ID) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE SET NULL
);
...
CREATE TABLE TB_CLUB (
	...
	CONSTRAINT CLUB_PRE_FK FOREIGN KEY(PRESIDENT_ID) REFERENCES TB_STUDENT(STD_ID) ON DELETE SET NULL,
	CONSTRAINT CLUB_MAN_FK FOREIGN KEY(CLUB_MANAGER_ID) REFERENCES TB_PROFESSOR(PROF_ID) ON DELETE SET NULL
);
...
CREATE TABLE TB_CLUB_MEMBER (
	...
	CONSTRAINT CM_STD_FK FOREIGN KEY(STD_ID) REFERENCES TB_STUDENT(STD_ID) ON DELETE CASCADE,
	CONSTRAINT CM_CLUB_FK FOREIGN KEY(CLUB_ID) REFERENCES TB_CLUB(CLUB_ID) ON DELETE CASCADE
);
...
CREATE TABLE TB_PAYMENT_HISTORY (
	...
	PAY_YEAR VARCHAR2(4 BYTE), -- 등록금 납부 연도
	PAY_SEMESTER CHAR(1 BYTE) NOT NULL, -- 등록금 납부 학기
	TUITION_TOTAL NUMBER, -- 등록금 총액
	PAY_TOTAL NUMBER, -- 납부 총액
	LAST_PAYDATE DATE DEFAULT CURRENT_DATE, -- 마지막 납부 일자
	CONSTRAINT PAY_SEM_CK CHECK(PAY_SEMESTER IN('1', '2')),
	CONSTRAINT PAY_PK PRIMARY KEY(STD_ID, PAY_YEAR, PAY_SEMESTER),
	CONSTRAINT PAY_AMOUNT_CK CHECK(PAY_TOTAL <= TUITION_TOTAL),
	CONSTRAINT PAY_STD_FK FOREIGN KEY(STD_ID) REFERENCES TB_STUDENT(STD_ID) ON DELETE CASCADE
);